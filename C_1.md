...
- A perfect discriminator would classify all real data as real (D(x) = 1) and all generated data as fake (D(G(z)) = 0)
...
The notation min₍G₎ max₍D₎ V(D,G) means: "Find the generator G that minimizes the maximum value that any discriminator D
could achieve."
